...
def solve(problem):
    def resolve(problem): #resolves parenthesis into a readable equation
        ...
        for char in problem:
            if char == "(":
                ...
                if pLevel > 0:
                    microproblem=microproblem+char
            elif pLevel > 0: #put parenthesis into micropreblem
                microproblem=microproblem+char
            elif char.isalnum() == False and pLevel == 0: #['+','-','*','/','%','^','.'] if not a num, and not in parenthesis assume operator
                a=str(a)+char
    ...
    for char in problem:
        if char.isdigit() and f == -1:
            ...
            d=0
            unary=False
        elif char.isdigit() and f > -1: #if we are beyond a '.'
            if 0 <= i < len(nums):
                if negative == True:
            ...
            if doMath(nums)==solve(problem[ii+1:]): #is the first side = to the second side?
                ...
                return False
        elif char != " ": #should not trigger unless something other than +-*/^%= is typed
            return 'Error: "'+char+'" not a valid operator'
        ii+=1
    oper=0
    for elem in nums:
        if isinstance(elem, int) or isinstance(elem, float): #if is int or float reset operator counter
            oper=0
            continue
